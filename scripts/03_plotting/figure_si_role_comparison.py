...
from cumulative_advantage_brokerage.visuals import\
    plot_bf_role_comparison, plot_br_role_comparison
...
def main():
    ...
    with CumAdvBrokSession(engine) as session:
        for key_cmp, f_get_test_results in zip(
            ...
            tpl_str_files.append(os.path.join(
                config[ARG_PATH_CONTAINER_OUTPUT], f"si_role_{key_cmp[3:-1]}.pdf"))

    fig, _, l_legends = plot_bf_role_comparison(
        tpl_d_tests=tpl_d_tests[0]
    )
    ...
    fig, a_ax, l_legends = plot_br_role_comparison(
        tpl_d_tests_br_cmp_role=tpl_d_tests[1],
    ...
    fig, _, l_legends = plot_br_role_comparison(
        tpl_d_tests_br_cmp_role=tpl_d_tests[2],
